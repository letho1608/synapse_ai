...
from synapse.helpers import pretty_print_bytes, pretty_print_bytes_per_second
...
class TopologyViz:
  def __init__(self, chatgpt_api_endpoints: List[str] = [], web_chat_urls: List[str] = []):
    ...
    # Main area: no Panel (no box) to avoid ╭─╮ │ ╰─╯ in terminal
    ...
    self.layout["main"].update(self._generate_main_layout())
  ...
  def update_prompt(self, request_id: str, prompt: Optional[str] = None):
    # Clean prompt: remove special tokens and format for display
    if prompt:
      cleaned_prompt = self._clean_prompt_for_display(prompt)
    else:
      cleaned_prompt = prompt
    self.requests[request_id] = [cleaned_prompt, self.requests.get(request_id, ["", ""])[1]]
    self.refresh()
  
  def _clean_prompt_for_display(self, prompt: str) -> str:
    """Remove special tokens and format prompt for better display in TUI"""
    import re
    if not prompt:
      return prompt
    ...
    # Extract role and content from header format: <|start_header_id|>role<|end_header_id|>content
    # Pattern: <|start_header_id|>role<|end_header_id|>content<|eot_id|>
    parts = []
    pattern = r'<\|start_header_id\|>([^<]+)<\|end_header_id\|>([^<]*?)(?=<\|start_header_id\|>|<\|eot_id\|>|$)'
    ...
    if parts:
      ...
      # Fallback: remove all special tokens
      cleaned = re.sub(r'<\|start_header_id\|>([^<]+)<\|end_header_id\|>', r'\1:', cleaned)
    ...
    # If empty after cleaning, return a simplified version
    if not result:
      # Try to extract just the user message
      user_match = re.search(r'user[:\s]+(.+?)(?=assistant|$)', prompt, re.IGNORECASE | re.DOTALL)
      if user_match:
